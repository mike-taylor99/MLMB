...
export function useInfiniteTeams(sport?: Sport, limit = 50) {
  return useInfiniteQuery<
    TeamsListResponse,
    Error,
    InfiniteData<TeamsListResponse>,
    readonly unknown[],
    string | undefined
  >({
    queryKey: queryKeys.teams({ sport, limit }),
    queryFn: ({ pageParam }) =>
      fetchTeams({ sport, limit, after_id: pageParam }),
...
export function usePredictions(params: ListPredictionsParams) {
  return useInfiniteQuery<
    PredictionListResponse,
    Error,
    InfiniteData<PredictionListResponse>,
    readonly unknown[],
    string | undefined
  >({
  ...
  return useMutation<Bracket, Error, CreateBracketRequest>({
    ...
    onSuccess: (data, vars) => {
      ...
      qc.invalidateQueries({
        queryKey: queryKeys.brackets(vars.tournament_id),
      });
    },
  });
}
...
export function useUpdateBracket() {
  ...
  return useMutation<
    Bracket,
    Error,
    { bracketId: string; body: UpdateBracketRequest }
  >({
    ...
    onSuccess: (data) => {
      ...
      qc.invalidateQueries({
        queryKey: queryKeys.brackets(data.tournament_id),
      });
    },
  });
}
