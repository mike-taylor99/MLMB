      ...
      <Card>
        <CardContent className="py-4 space-y-3">
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 flex items-center gap-2 min-w-0 justify-end">
              <div className="text-right min-w-0">
                <Link
                  to={`/teams/${prediction.home_team}`}
                  className={`truncate text-sm hover:underline ${homeWins ? "font-semibold" : ""}`}
                >
                  {homeLabel}
                </Link>
          ...
          <div className="flex items-center gap-2">
            <Badge
              variant={homeWins ? "default" : "secondary"}
          ...
          <div className="flex items-center justify-between text-[11px] text-muted-foreground">
            <span>
              ...
              {prediction.model} · Span {prediction.span}
            </span>
    ...
    <Card className="overflow-hidden">
      <CardContent className="py-6">
        <div className="grid grid-cols-[1fr_auto_1fr] items-start gap-2 min-w-0">
          <div className="flex flex-col items-center space-y-2 min-w-0">
            ...
            <div className="text-center w-full px-2">
              ...
              <p className="text-xs text-muted-foreground h-4 truncate">
                {homeTeam && homeTeam.meta.name !== homeTeam.meta.school
                  ? homeTeam.meta.name
                  : "\u00A0"}
              </p>
              {fmtDate(prediction.home_last_played) && (
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  thru {fmtDate(prediction.home_last_played)}
                </p>
              )}
            </div>
            <Badge
          ...
          <div className="text-2xl font-bold text-muted-foreground self-center">
            vs
          </div>
          <div className="flex flex-col items-center space-y-2 min-w-0">
            ...
            <div className="text-center w-full px-2">
              ...
              <p className="text-xs text-muted-foreground h-4 truncate">
                {awayTeam && awayTeam.meta.name !== awayTeam.meta.school
                  ? awayTeam.meta.name
                  : "\u00A0"}
              </p>
              {fmtDate(prediction.away_last_played) && (
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  thru {fmtDate(prediction.away_last_played)}
                </p>
              )}
            </div>
            <Badge
