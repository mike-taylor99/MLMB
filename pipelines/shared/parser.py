...
from shared.constants import META_LABELS, STAT_LABELS, LATEST, RENAME_OPPOSING_COLS
...
def create_basic_gamelog(school_key: str, season: int, is_womens: bool = False) -> None:
    ...
    csv_path = get_team_season_file_path(school_key, season, f"{school_key}_basic.csv", is_womens)
...
def create_advanced_gamelog(school_key: str, season: int, is_womens: bool = False) -> None:
    ...
    csv_path = get_team_season_file_path(school_key, season, f"{school_key}_advanced.csv", is_womens)
    df.to_csv(csv_path, index=False)
...
def combine_basic_advanced(school_key: str, season: int, is_womens: bool = False) -> None:
    """Merge basic and advanced gamelog CSVs into a single file."""
    basic_path = get_team_season_file_path(school_key, season, f"{school_key}_basic.csv", is_womens)
    adv_path = get_team_season_file_path(school_key, season, f"{school_key}_advanced.csv", is_womens)

    basic_df = pd.read_csv(basic_path)
    adv_df = pd.read_csv(adv_path)

    merge_cols = ["Rk", "Gtm", "Date", "Location", "Opp key", "Type", "Rslt", "Tm", "Opp", "OT", "eFG%"]
    ...
    out_path = get_team_season_file_path(school_key, season, f"{school_key}_merged.csv", is_womens)
...
def generate_moving_averages(
    school_key: str, season: int, span: int = 5,
    keep_latest: bool = False, is_womens: bool = False
) -> None:
    """Compute SMA/CMA/EMA for all stat columns and save to CSV."""
    file_path = get_team_season_file_path(school_key, season, f"{school_key}_merged.csv", is_womens)
    ...
    out_path = get_team_season_file_path(school_key, season, f"{school_key}_{span}ma.csv", is_womens)
...
def merge_opponent_data(
    ...
    file_path = get_team_season_file_path(school_key, season, f"{school_key}_{span}ma.csv", is_womens)
    df = pd.read_csv(file_path)
    ...
    for _, row in df.iterrows():
        try:
            ...
            opp_path = get_team_season_file_path(opp_key, season, f"{opp_key}_{span}ma.csv", is_womens)
            ...
            cur_df = df[
                (df["Opp key"] == game.get("Opp key")) & (df["Date"] == game.get("Date"))
            ]
    ...
    out_path = get_team_season_file_path(school_key, season, f"{school_key}_{span}span_full.csv", is_womens)
    merged.to_csv(out_path, index=False)
