...
const SPAN_LABEL: Record<number, string> = { 3: '3-game', 5: '5-game', 7: '7-game' }
...
function PredictionDetail({
  ...
  const sorted = [...predictions.scenarios].sort(
    (a, b) => a.span - b.span || (a.topIsHome === b.topIsHome ? 0 : a.topIsHome ? -1 : 1),
  )
  ...
  return (
    <div className="space-y-2.5">
      ...
      {/* Team probabilities with bar */}
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs">
          <span className={cn('font-medium truncate mr-2', overallAvg >= 0.5 && 'text-primary')}>
            {topSchool}
          </span>
          <span className={cn('tabular-nums font-semibold shrink-0', overallAvg >= 0.5 && 'text-primary')}>
            {topPct}%
          </span>
        </div>
      ...
      <div className="pt-1 border-t">
        ...
        >
          <span>
            {predictions.scenarios.length} model runs · favors <span className="font-semibold text-foreground">{favored}</span>
          </span>
              ...
              return (
                <div key={i} className="flex items-center justify-between text-[11px]">
                  <span className="text-muted-foreground">
                    ...
                    {' · '}
                    <span className="opacity-70">{sc.topIsHome ? topSchool : bottomSchool} home</span>
                  </span>
                  <span className="tabular-nums font-medium ml-2 shrink-0">
                    <span className={cn(sc.topWinProb >= 0.5 ? 'text-primary' : 'text-muted-foreground')}>
                      {pct}%
                    </span>
...
interface PickMatchupProps {
  ...
  onRequestPredictions?: (gameKey: string, topTeam: string, bottomTeam: string) => void
...
export function PickMatchup({
  ...
  const avgTop = predictions ? averageTopProb(predictions.scenarios) : undefined
